...
class ClockPunchTest extends TestCase {
    ...
    public function test_the_four_punches_fill_their_own_columns(): void {
        ...
        // Break times are an admin entry, never punched here.
        $this->assertNull($record->break_out);
    }

    public function test_break_times_cannot_be_punched_from_this_screen(): void {
        $admin = User::factory()->create();
        $employee = $this->employee();
        $this->punch($admin, $employee, 'in', '09:00')->assertOk();

        $this->punch($admin, $employee, 'break-out', '12:00')->assertNotFound();
    }
